...
| | |
|---|---|
| **Runs today** | 33 Go packages, 372 tests, race-clean — 32 packages carry tests and the thirty-third, `cmd/sdev1-ql`, is proved by its records' fences running the built binary. Two binaries, `sdev1-addr` and `sdev1-ql`. |
...
| **Honestly measured** | 256 mutants killed across the corpus, 18 recorded as *survived*. ★ Those rows are kept rather than deleted even after the test that let one through is strengthened — a mutant that lived is the record of what the suite could not see. Seven found claims that nothing was holding: one was a real bug, and two showed claims **no test in this design can falsify**, which were withdrawn or marked unprovable rather than propped up. |
...
docs/
  ...
  diagrams/               the schematics on this page
    adr/                    the decision corpus — thirty-one Accepted records
