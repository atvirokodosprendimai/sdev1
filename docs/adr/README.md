...
Eleven records are **Accepted and implemented**: ADR-001 through ADR-008,
ADR-016, ADR-017 and ADR-019 — the last with one of its two tasks done, which its
task index states rather than implies. Their `Governs:` paths resolve, their tasks carry tool-written
