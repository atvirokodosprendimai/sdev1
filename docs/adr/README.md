...
Eighteen records are **Accepted and implemented**: ADR-001 through ADR-012,
ADR-015 through ADR-020 except ADR-013 and ADR-014.

The two remaining — ADR-013's MCP server and ADR-014's FUSE projection — are
both projections over ADR-011's evaluator, and the evaluator needs a storage
engine (`BACKLOG.md` §20 and §12). They are named below rather than written,
because a record whose every task would be `pending` on the same missing thing
says less than one sentence naming what is missing. Two carry one of their two tasks — ADR-009's
...
| [ADR-018](ADR-018-read-ahead.md) | Read-ahead is a budgeted hint that fetches the nearest `k` and hedges only on a straggler | **Accepted** (the plan is done; fetching and caching are `BACKLOG.md` §18/§24) |
